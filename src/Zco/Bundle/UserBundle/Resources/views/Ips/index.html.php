...
<ul>
    ...
    <li>
        Voir tous les bans de l'adresse IP suivante :
        <form method="get" action="" class="form-inline">
            <input type="text" name="ip" id="ip"
                   value="<?php echo isset($_GET['ip']) ? htmlspecialchars($_GET['ip']) : '' ?>"/>
...
<?php if (verifier('ips_bannir')): ?>
    <p class="bold center">
        <a href="<?php echo $view['router']->path('zco_user_ips_ban') ?>">Bannir une adresse IP</a>
    </p>
<?php endif ?>
    ...
    <table class="table table-striped">
        ...
        <?php foreach ($ListerIPs as $cle => $valeur) { ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo $view['router']->path('zco_user_profile', ['id' => $valeur['utilisateur_id'], 'slug' => rewrite($valeur['utilisateur_pseudo'])]) ?>">
                        <?php echo htmlspecialchars($valeur['utilisateur_pseudo']) ?>
                    </a>
                </td>
                <td class="center <?php echo $valeur['ip_fini'] ? 'rouge' : 'vertf' ?>">
                    <?php echo dateformat($valeur['ip_date_debut']) ?>
                </td>
                <td class="center <?php echo $valeur['ip_fini'] ? 'rouge' : 'vertf' ?>">
                    <?php
                    ...
                    if ($valeur['ip_duree_restante'] > 0 && !$valeur['ip_fini']) { ?><br/>
                        (dans <?php echo $valeur['ip_duree_restante'] ?> jour<?php echo pluriel($valeur['ip_duree_restante']) ?>)<?php } ?>
                </td>
                ...
                <td class="center">
                    <?php if (verifier('ips_bannir')): ?>
                        <?php if (!$valeur['ip_fini']): ?>
                            <a href="<?php echo $view['router']->path('zco_user_ips_index', ['cancel' => $valeur['ip_id'], 'tk' => $_SESSION['token']]) ?>"
                               title="Mettre fin à ce ban">
                                <img src="/img/refuser.png" alt="Mettre fin"/></a>
                        <?php else: ?>
                            <a href="<?php echo $view['router']->path('zco_user_ips_index', ['delete' => $valeur['ip_id'], 'tk' => $_SESSION['token']]) ?>"
                               title="Supprimer">
                                <img src="/img/supprimer.png" alt="Supprimer"/></a>
                        <?php endif ?>
                    <?php else: ?>
                        -
                    <?php endif ?>
