...
<p>
	Vous vous apprêtez à répondre à la demande de <strong><a href="<?php echo $view['router']->path('zco_user_profile', array('id' => $query->getUserId(), 'slug' => rewrite($query->getUser()->getUsername()))) ?>"><?php echo htmlspecialchars($query->getUser()) ?></a></strong>.
...
<form method="post" action="<?php echo $view['router']->path('zco_user_admin_newPseudoAnswer', array('id' => $query->getId())) ?>" class="form-horizontal">
