...
>
	...
	<xsl:template match="titre1">
		<xsl:variable name="ancre" select="php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::ancre', .)" />
		<xsl:choose>
			<xsl:when test="$ancre != ''">
				<h3 id="{$ancre}">
					<xsl:apply-templates/>
	...
	<xsl:template match="lien">
		<xsl:choose>
			<xsl:when test="@url and php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::verifierLien', @url)">
				<xsl:element name="a">
			...
			<xsl:when test="php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::verifierLien', .)">
				<xsl:element name="a">
					<xsl:attribute name="href">
						<xsl:choose>
	...
	<xsl:template match="citation">
		<span class="citation">
			<xsl:choose>
				<xsl:when test="not(@rid) and @lien and php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::verifierLien', @lien)">
					<xsl:element name="a">
						<xsl:attribute name="href">
							<xsl:choose>
				...
				<xsl:when test="@rid">
					<xsl:variable name="rid" select="php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::citationRid', @rid)"/>
					<xsl:choose>
						<xsl:when test="$rid">
	...
	<xsl:template name="codeTitre">
		...
		<xsl:value-of select="php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::nomCode', @type)"/>
		<xsl:if test="@titre">
			<xsl:text> - </xsl:text>
	...
	<xsl:template match="code">
		...
		<xsl:variable name="type" select="php:functionString('\Zco\Bundle\CoreBundle\Parser\CoreFeature::typeCode', @type)"/>
		<div class="code2{$type}">
			<xsl:element name="zcode-code">
				<xsl:value-of select="php:function('\Zco\Bundle\CoreBundle\Parser\CoreFeature::colorerCode', .)"/>
			</xsl:element>
		</div>
	</xsl:template>
