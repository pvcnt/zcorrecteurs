 ...
 */
class FilesFeature extends AbstractFeature
{
    private $filesystem;
    private $imagine;
    ...
    public function processDom(\DOMDocument $doc, array $options): \DOMDocument
    {
        ...
        $relatedToEntity = isset($options['files.entity_id']) && isset($options['files.entity_class']);
        ...
        if ($relatedToEntity) {
            ...
            $rows = \Doctrine_Query::create()
                ...
                ->from('FileUsage u')
                ->where('entity_class = ?', $options['files.entity_class'])
                ->andWhere('entity_id = ?', $options['files.entity_id'])
                ->andWhere('part = ?', $part)
            ...
            if (!$file) {
                ...
                if ($relatedToEntity && $thumbnail) {
                    if (!isset($used['thumbnails'][$thumbnail['id']])) {
                        ...
                        $usage['entity_class'] = $options['files.entity_class'];
                        $usage['entity_id'] = $options['files.entity_id'];
                        $usage->save();
