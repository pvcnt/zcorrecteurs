...
class awGDGradientDriver
{
    ...
    private function drawFilledTriangleVertically(awGradient $gradient, awPolygon $polygon)
    {
        ...
        for ($y = round($yMin); $y < round($yMax); $y++) {
            ...
            if (count($toX) === 1) {
                $fillLine = new awLine(new awPoint($fromX, $y), new awPoint($toX[0], $y));
            } else {
                $line1 = new awLine(new awPoint($fromX, $y), new awPoint($toX[0], $y));
                $line2 = new awLine(new awPoint($fromX, $y), new awPoint($toX[1], $y));
    ...
    private function drawFilledTriangleHorizontally(awGradient $gradient, awPolygon $polygon)
    {
        ...
        for ($x = round($xMin); $x < round($xMax); $x++) {
            ...
            if (count($toY) === 1) {
                $fillLine = new awLine(new awPoint($x, $fromY), new awPoint($x, $toY[0]));
            } else {
                $line1 = new awLine(new awPoint($x, $fromY), new awPoint($x, $toY[0]));
                $line2 = new awLine(new awPoint($x, $fromY), new awPoint($x, $toY[1]));
                if ($line1->getSize() < $line2->getSize()) {
                    $fillLine = $line1;
                } else {
                    $fillLine = $line2;
